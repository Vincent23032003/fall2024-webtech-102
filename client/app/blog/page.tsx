...
export default function BlogPage() {
  ...
  // Récupérez les articles (avec recherche si nécessaire)
  useEffect(() => {
    const fetchArticles = async () => {
      ...
      try {
        ...
        // Créez une requête dynamique pour inclure la recherche si nécessaire
        let query = supabase
          .from("articles")
          .select(
            `
        ...
        if (searchTerm.trim()) {
          query = query.textSearch("search", searchTerm.trim(), {
            type: "websearch",
          });
        }
  ...
  const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setCurrentPage(1); // Réinitialisez à la première page après une nouvelle recherche
  };
          ...
          {articles.map((article) => (
            ...
            >
              ...
              <p className="text-sm text-gray-500">
                Publish on {new Date(article.created_date).toLocaleDateString()} par {article.users?.username || "Auteur inconnu"}
              </p>
